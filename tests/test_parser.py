...
def test_parser():
    ...
    assert is_correct_syntax_text('let d = get_vertices map (lambda (a, b, c) -> step a use c) use l;')
    assert is_correct_syntax_text('let d = get_vertices filter (lambda (a, b, c) -> step a use c) use l;')
    assert is_correct_syntax_text('let d = set_final {1...100} in (filter (lambda (a, b, c) -> step a use c) use l);')
